...
-- Políticas de seguridad (drop if exists first)
DROP POLICY IF EXISTS "Users can view public activities" ON published_activities;
CREATE POLICY "Users can view public activities"
  ON published_activities FOR SELECT
  TO authenticated
  USING (is_public = true OR auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own activities" ON published_activities;
CREATE POLICY "Users can insert their own activities"
  ON published_activities FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own activities" ON published_activities;
CREATE POLICY "Users can update their own activities"
  ON published_activities FOR UPDATE
  TO authenticated
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own activities" ON published_activities;
...
CREATE INDEX IF NOT EXISTS idx_published_activities_user_id ON published_activities(user_id);
CREATE INDEX IF NOT EXISTS idx_published_activities_activity_date ON published_activities(activity_date DESC);
CREATE INDEX IF NOT EXISTS idx_published_activities_public ON published_activities(is_public) WHERE is_public = true;
...
DROP TRIGGER IF EXISTS update_published_activities_updated_at ON published_activities;
